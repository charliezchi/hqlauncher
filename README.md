...
如果你从源码仓库使用，一行命令完成编译并注册到 PATH。`make` 会先 `clean` 再 `build`，`build` 成功后会自动将 `hqlauncher.exe` 复制到 `%APPDATA%\hqlauncher\` 并添加到用户 PATH：

```bat
make        # 先 clean 再 build + 自动注册
make build  # 仅 build + 自动注册
```
...
| 命令           | 说明                                              |
| -------------- | ------------------------------------------------- |
| `make`         | 先 `clean` 再 `build`，成功后自动复制 exe 并注册 PATH |
| `make build`   | 打包 `hqlauncher.exe`，成功后自动复制 exe 并注册 PATH |
...
├── make.bat             # 构建 / 清理统一管理，build 成功后自动注册 PATH
├── .gitignore
├── LICENSE
└── README.md
```
